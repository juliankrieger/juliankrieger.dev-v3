...
One solution for this is to convert `0x496c5962` into an ASCII string. Most ASCII characters fall into a range between 0 and 255, a.k.a 2 to the power of 8 possible characters, one one byte. Since a single hex digit stores 4 bit of information, we split `0x496c5962` into two packets for out character candidates we want to pipe into `stack-one` (after the padding). This leaves us with `0x49` `0x6c` `0x59` and `0x62`. Translate that into human readable ASCII characters and you get `IlYb`. 
